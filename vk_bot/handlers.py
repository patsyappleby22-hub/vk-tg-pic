...
RESERVED_TEXTS = (
    MENU_TEXTS | SETTINGS_TEXTS | STOP_TEXTS
    | CHAT_TEXTS | BALANCE_TEXTS
)
...
def register_handlers(bot: Bot, vertex_service: VertexAIService) -> None:
    ...
    async def cmd_start(message: Message):
        ...
        # First message refreshes the persistent (sticky) keyboard;
        # second message carries the inline "🌐 Открыть веб-чат" button.
        await message.answer(
            _build_vk_menu_text(first_name, generations, credits, blocked),
            keyboard=get_persistent_keyboard(),
        )
        await message.answer(
            "🌐 Веб-версия чата — откроется с уже подставленным аккаунтом, "
            "код придёт в этот диалог автоматически.",
            keyboard=get_web_chat_inline_keyboard(uid),
        )
    ...
    async def cmd_menu(message: Message):
        ...
        # Persistent keyboard stays sticky from /start, so attach the
        # inline web-chat button right to the menu text itself.
        await message.answer(
            _build_vk_menu_text(first_name, generations, credits, blocked),
            keyboard=get_web_chat_inline_keyboard(uid),
        )
