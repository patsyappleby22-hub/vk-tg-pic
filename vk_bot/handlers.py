...
WEB_CHAT_TEXTS = {"🌐 веб-чат", "🌐 Веб-чат", "веб-чат", "вебчат", "web", "webchat"}
RESERVED_TEXTS = (
    MENU_TEXTS | SETTINGS_TEXTS | STOP_TEXTS
    | CHAT_TEXTS | BALANCE_TEXTS | WEB_CHAT_TEXTS
)
...
def register_handlers(bot: Bot, vertex_service: VertexAIService) -> None:
    ...
    @bot.on.message(text=list(WEB_CHAT_TEXTS))
    async def cmd_web_chat(message: Message):
        """Issue a fresh login code and DM the user a prefilled web-chat link."""
        from bot.web_chat import issue_login_code

        uid = message.from_id
        code, err = await issue_login_code(uid, "vk")
        if not code:
